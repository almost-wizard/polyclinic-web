...
public class EditUserCommand extends ActionCommand {
    ...
    public void execute() throws ServletException, IOException {
        ...
        User newUser = CreateUserCommand.prepareUser(request);
        int id = Integer.parseInt(request.getParameter(PARAM_NAME_ID));

        boolean isEdited = userDao.edit(id, newUser);

        if (!isEdited) {
            request.setAttribute("errorMessage", MessageManager.get("user.edit.error"));
            redirect(Command.GET_EDIT_USER_FORM);
        } else {
